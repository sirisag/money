...
import 'package:money/utils/constants.dart'; // Import constants
...
class FileExportService {
    ...
    try {
      ...
      if (monkTransactionsData != null) {
        ...
        for (var txData in monkTransactionsData) {
          ...
          final existingTransaction =
              await _dbHelper.getTransaction(finalTransaction.uuid);
      ...
      if (tripExpensesData != null) {
        for (var txData in tripExpensesData) {
          ...
          final finalExpense = newExpenseForTreasurer.uuid ==
                  originalTransaction.uuid
              ? newExpenseForTreasurer
              : newExpenseForTreasurer.copyWith(uuid: originalTransaction.uuid);

          final existingExpenseTransaction =
              await _dbHelper.getTransaction(finalExpense.uuid);
  ...
  Future<String?> exportTreasurerUpdateToDriver({
    ...
    try {
      ...
      final List<Transaction> reconciledDriverTransactions =
          await _dbHelper.getTransactionsByStatusAndProcessor(
              TransactionStatus.reconciledByTreasurer, treasurerPrimaryId);
      ...
      final DriverAdvanceAccount? driverAdvanceAccount =
          await _dbHelper.getDriverAdvance(driverToUpdate.primaryId);
      ...
      final dataToExport = {
        ...
        'updatedMonkList':
            allMonks.map((monk) => monk.toMap(forFileExport: true)).toList(),
      };
  ...
  String? decryptData(String encryptedTextWithIv, String treasurerPrimaryId,
    ...
    return _encryptionService.decryptData(
        encryptedTextWithIv, treasurerPrimaryId, treasurerSecondaryId);
  }
}
