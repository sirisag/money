...
import 'package:money/models/transaction_model.dart' as model;
...
class DatabaseHelper {
  ...
  Future<List<User>> getAllUsers({sqflite.Transaction? txn}) async {
    ...
    final List<Map<String, dynamic>> maps =
        await dbClient.query(tableUsers, orderBy: 'role ASC, displayName ASC');
  ...
  Future<bool> isSecondaryIdTaken(String secondaryId,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    final List<Map<String, dynamic>> maps = await dbClient.query(
      tableUsers,
  ...
  Future<int> insertTransaction(model.Transaction transaction,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    return await dbClient.insert(tableTransactions, transaction.toMap(),
        conflictAlgorithm: sqflite.ConflictAlgorithm.fail);
  }

  Future<model.Transaction?> getTransaction(String uuid,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    final List<Map<String, dynamic>> maps = await dbClient.query(
      tableTransactions,
  ...
  Future<List<model.Transaction>> getAllTransactions(
    ...
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    final List<Map<String, dynamic>> maps =
        await dbClient.query(tableTransactions, orderBy: 'timestamp DESC');
    return List.generate(maps.length, (i) {
  ...
  Future<int> updateTransactionStatus(
      String uuid, model.TransactionStatus status,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    return await dbClient.update(
      tableTransactions,
  ...
  Future<List<model.Transaction>> getTransactionsForMonkAndDriver(
      String monkPrimaryId, String driverPrimaryId,
      {model.TransactionStatus? status, sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    String whereClause =
  ...
  Future<List<model.Transaction>> getTransactionsByStatusAndProcessor(
      model.TransactionStatus status, String processedByPrimaryId,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    final List<Map<String, dynamic>> maps = await dbClient.query(
      tableTransactions,
  ...
  Future<List<model.Transaction>> getTempleFundTransactionsForMonth(
      String treasurerPrimaryId, DateTime monthYear,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
  ...
  Future<List<model.Transaction>> getMonkFinancialHistory(
      String monkPrimaryId, String treasurerPrimaryId,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    final List<Map<String, dynamic>> maps = await dbClient.query(
      tableTransactions,
  ...
  Future<List<model.Transaction>> getDriverFinancialHistory(
      String driverPrimaryId, String treasurerPrimaryId,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    final List<Map<String, dynamic>> maps = await dbClient.query(
      tableTransactions,
      ...
      whereArgs: [
        ...
        treasurerPrimaryId,
        model.TransactionStatus.reconciledByTreasurer.name,
        model.TransactionType.TEMPLE_INCOME
  ...
  Future<int> insertOrUpdateMonkFundAtTreasurer(MonkFundAtTreasurer fund,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    return await dbClient.insert(tableMonkFundsAtTreasurer, fund.toMap(),
        conflictAlgorithm: sqflite.ConflictAlgorithm.replace);
  }
  ...
  Future<int> insertOrUpdateMonkFundAtDriver(MonkFundAtDriver fund,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    return await dbClient.insert(tableMonkFundsAtDriver, fund.toMap(),
        conflictAlgorithm: sqflite.ConflictAlgorithm.replace);
  }
  ...
  Future<List<MonkFundAtDriver>> getMonkFundsByDriver(String driverPrimaryId,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    final List<Map<String, dynamic>> maps = await dbClient.query(
      tableMonkFundsAtDriver,
  ...
  Future<int> insertOrUpdateDriverAdvance(DriverAdvanceAccount advance,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    return await dbClient.insert(tableDriverAdvanceAccounts, advance.toMap(),
        conflictAlgorithm: sqflite.ConflictAlgorithm.replace);
  }

  Future<DriverAdvanceAccount?> getDriverAdvance(String driverPrimaryId,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    final List<Map<String, dynamic>> maps = await dbClient.query(
  ...
  Future<int> insertOrUpdateTempleFund(TempleFundAccount fund,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    return await dbClient.insert(tableTempleFundAccounts, fund.toMap(),
        conflictAlgorithm: sqflite.ConflictAlgorithm.replace);
  }

  Future<TempleFundAccount?> getTempleFund(String treasurerPrimaryId,
      {sqflite.Transaction? txn}) async {
    final sqflite.DatabaseExecutor dbClient = txn ?? await database;
    final List<Map<String, dynamic>> maps = await dbClient.query(
