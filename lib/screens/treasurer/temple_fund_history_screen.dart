...
class _TempleFundHistoryScreenState extends State<TempleFundHistoryScreen> {
  ...
  DateTime _selectedMonthYear = DateTime(
      DateTime.now().year, DateTime.now().month, 1); // Default to current month
  ...
  Future<void> _loadTempleFundHistory() async {
    ...
    try {
      final transactionsFromDb =
          await _dbHelper.getTempleFundTransactionsForMonth(
              _currentTreasurerId!, _selectedMonthYear);
          ...
          children: <Widget>[
            IconButton(
              ...
              onPressed: _isLoading
                  ...
                  : () {
                      setState(() {
                        _selectedMonthYear = DateTime(_selectedMonthYear.year,
                            _selectedMonthYear.month - 1, 1);
                      });
                      _loadTempleFundHistory();
                    },
            ),
