...
import 'package:money/models/transaction_model.dart';
...
class _RecordMonkFundAtTreasurerDialogState
    ...
    try {
      ...
      for (var monkMap in monkMaps) {
        ...
        final lastActivity = lastActivityString != null
            ...
            : null;
        tempSortableMonks
            .add(MonkForDialog(monk: monkUser, lastActivity: lastActivity));
      }
  ...
  Future<void> _submitTransaction() async {
    ...
    bool? confirmSave = await showDialog<bool>(
      ...
      builder: (BuildContext innerDialogContext) {
        return AlertDialog(
          ...
          content: Text('พระ: ${_selectedMonk!.displayName}\n'
    ...
    final transaction = Transaction(
      ...
      sourceAccountId:
          _isDeposit ? _selectedMonk!.primaryId : _currentTreasurerId!,
      destinationAccountId:
          _isDeposit ? _currentTreasurerId! : _selectedMonk!.primaryId,
    ...
    try {
      ...
      await db.transaction((txn) async {
        ...
        MonkFundAtTreasurer? monkFund = await _dbHelper.getMonkFundAtTreasurer(
            _selectedMonk!.primaryId, _currentTreasurerId!,
            txn: txn);
        ...
        await _dbHelper.updateTempleFundBalance(
            _currentTreasurerId!, _isDeposit ? amount : -amount,
            txn: txn);
      });
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    DropdownButtonFormField<bool>(
                      ...
                      items: const [
                        DropdownMenuItem(
                            value: true, child: Text('พระฝากเงิน')),
                        DropdownMenuItem(
                            value: false, child: Text('พระเบิกเงิน')),
                      ],
                      onChanged: (value) {
                        if (value != null) {
                          setState(() {
