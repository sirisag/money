...
import 'package:money/models/user_model.dart'; // For User model to display names
import 'package:money/models/account_balance_models.dart'; // Import AccountBalance models
...
class _ReconcileTransactionsScreenState
  ...
  Future<void> _reconcileTransaction(Transaction transaction) async {
    ...
    if (confirmReconcile == null || !confirmReconcile) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('ยกเลิกการกระทบยอดรายการ')),
      );
      return;
    }
    ...
    try {
      await db.transaction((txn) async {
        ...
        if (transaction.type == TransactionType.DEPOSIT_FROM_MONK_TO_DRIVER ||
          ...
          await _dbHelper.updateTempleFundBalance(
              _currentTreasurerId!, -transaction.amount,
              txn: txn);
        } else if (transaction.type ==
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('กระทบยอดรายการสำเร็จ!')),
      );
    ...
    return Scaffold(
      ...
      body: _isLoading
          ...
          : _pendingTransactions.isEmpty
              ? const Center(
                  child: Text('ไม่มีรายการที่รอการกระทบยอด',
