...
import 'package:money/services/database_helper.dart';
// Import a screen for adding/editing a driver
import 'package:money/screens/treasurer/add_edit_driver_screen.dart';
// For file operations
import 'package:file_picker/file_picker.dart'; // For FilePicker
import 'package:money/services/file_export_service.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:share_plus/share_plus.dart'; // Import share_plus
import 'package:money/models/transaction_model.dart'; // For Transaction.create
import 'package:money/models/account_balance_models.dart'; // For AccountBalance models
import 'package:money/screens/treasurer/driver_financial_history_screen.dart'; // Import history screen
...
class DriverWithDetails {
  ...
  DriverWithDetails(
      {required this.driver, this.advanceAccount, this.lastActivityTimestamp});
}
...
class _ManageDriversScreenState extends State<ManageDriversScreen> {
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      final List<Map<String, dynamic>> driverMaps =
          await _dbHelper.getUsersByRoleWithLastActivity(UserRole.driver);
      ...
      for (var driverMap in driverMaps) {
        ...
        final lastActivity = lastActivityString != null
            ...
            : null;
        tempDriversWithDetails.add(DriverWithDetails(
            driver: driverUser,
  ...
  Future<InitialExportData?> _showInitialExportSetupDialog(User driver) async {
    ...
    final List<User> allMonks =
        await _dbHelper.getActiveMonks(); // Fetch only active monks
            ...
            child: Form(
              ...
              child: ListView(
                ...
                children: <Widget>[
                  ...
                  const Text("เงินที่พระฝากให้คนขับดูแล (ผ่านไวยาวัจกรณ์):",
            ...
            TextButton(
              ...
              onPressed: () {
                if (formKey.currentState!.validate()) {
                  ...
                  Navigator.of(dialogContext).pop(InitialExportData(
  ...
  Future<void> _proceedWithExport(User driver, int initialAdvanceAmount,
      Map<String, int> initialMonkDeposits) async {
    setState(() => _isLoading = true);
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      await db.transaction((txn) async {
        ...
        if (initialAdvanceAmount > 0) {
          ...
          await _dbHelper.updateTempleFundBalance(
              treasurerPrimaryId, -initialAdvanceAmount,
              txn: txn);

          DriverAdvanceAccount? dAccount =
              await _dbHelper.getDriverAdvance(driver.primaryId, txn: txn);
        ...
        for (var entry in initialMonkDeposits.entries) {
          ...
          if (amountToTransfer > 0) {
            ...
            if (mFund != null) {
              ...
              await _dbHelper.insertOrUpdateMonkFundAtTreasurer(mFund,
                  txn: txn);
            } else {
              ...
              await _dbHelper.insertOrUpdateMonkFundAtTreasurer(newMonkFund,
                  txn: txn);
              print(
      ...
      if (filePath != null && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('การส่งออกข้อมูลล้มเหลว')));
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<bool?> _showFinalInitialExportConfirmationDialog(
    ...
    monkDeposits.forEach((monkId, amount) {
      if (amount > 0) {
        ...
        final monkName =
            allMonks.firstWhere((m) => m.primaryId == monkId).displayName;
    ...
    return await showDialog<bool>(
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          title:
              Text('ยืนยันการส่งออกข้อมูลเริ่มต้นสำหรับ ${driver.displayName}'),
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                Text(
  ...
  Future<Map<String, dynamic>?> _showExportUpdateDialog(User driver) async {
    final TextEditingController newAdvanceController =
        TextEditingController(text: '0');
    ...
    for (var monk in allMonks) {
      final fund = await _dbHelper.getMonkFundAtTreasurer(
          monk.primaryId, currentTreasurerId);
      if (fund != null && fund.balance > 0) {
        monksUnderTreasurer.add(monk);
        monkTransferControllers[monk.primaryId] =
            TextEditingController(text: '0');
      }
    }
            ...
            child: Form(
              ...
              child: ListView(
                ...
                children: <Widget>[
                  ...
                  const SizedBox(height: 20),
                  const Text("โอนเงินปัจจัยพระให้คนขับรถดูแล:",
                      style: TextStyle(fontWeight: FontWeight.bold)),
    ...
    try {
      ...
      await db.transaction((txn) async {
        ...
        if (newAdvanceAmount > 0) {
          ...
          await _dbHelper.updateTempleFundBalance(
              treasurerPrimaryId, -newAdvanceAmount,
              txn: txn);
        ...
        for (var entry in monkTransfers.entries) {
          if (entry.value > 0) {
            ...
            monkFundsToTransferDataForFile
                .add({'monkPrimaryId': monkId, 'amount': amountToTransfer});
          }
        }
    ...
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        final Map<String, dynamic>? importResult =
            await _fileExportService.importDriverDataFromFile(
          filePath: filePath,
                  ...
                  actions: <Widget>[
                    ...
                    ElevatedButton(
                      ...
                      onPressed: () async {
                        ...
                        if (mounted) {
                          setState(() =>
                              _isImporting = true); // Show loading for commit
                        }
                        try {
                          final db = await _dbHelper.database;
                          await db.transaction((txn) async {
  ...
  Future<void> _showExportTypeDialog(User driver,
      {bool isInitialOnly = false}) async {
            ...
            children: <Widget>[
              if (isInitialOnly) // Show only initial export option
                ListTile(
                  ...
                  title:
                      const Text('ส่งข้อมูลเริ่มต้น (สำหรับตั้งค่าครั้งแรก)'),
                  onTap: () {
    ...
    if (exportType != null && mounted) {
      if (exportType == 'initial') {
        ...
        final List<User> allMonks =
            await _dbHelper.getActiveMonks(); // Fetch active monks
        // ignore: use_build_context_synchronously
        InitialExportData? exportData =
            await _showInitialExportSetupDialog(driver);
        ...
        if (exportData != null && mounted) {
          ...
          if (confirmedFinalExport == true) {
            ...
            _proceedWithExport(
                driver, exportData.driverAdvance, exportData.monkDeposits);
          }
        }
        ...
        Map<String, dynamic>? updateInput =
            await _showExportUpdateDialog(driver);
        if (updateInput != null && mounted) {
          ...
          bool? confirmed = await _showConfirmDataForUpdateDialog(
              driver, newAdvance, monkTransfers);
          if (confirmed == true && mounted) {
            await _proceedWithUpdateExport(
                driver, newAdvance, monkTransfers, currentTreasurerId);
          }
        }
      }
    }
  ...
  void _navigateToDriverHistory(
      User driver, DriverAdvanceAccount? advanceAccount) {
    Navigator.push(
      context,
      MaterialPageRoute(
          ...
          : _driversWithDetails.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      const Text('ยังไม่มีข้อมูลคนขับรถ',
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: CircleAvatar(
                          child: Text(driver.displayName.isNotEmpty
                              ? driver.displayName[0].toUpperCase()
                              : 'D')),
                      title: Text(driver.displayName),
                      subtitle: Text('Primary ID: ${driver.primaryId}\n'
            ...
            children: <Widget>[
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  onPressed:
                      _isImporting || _isLoading ? null : _importDataFromDriver,
                ),
              ),
                  ...
                  icon: _isLoading &&
                      ...
                      ? const SizedBox(
                          ...
                          child: CircularProgressIndicator(
                              strokeWidth: 2, color: Colors.white))
                      : const Icon(Icons
                          .sync_alt_outlined), // This was the second 'icon', now part of ternary
