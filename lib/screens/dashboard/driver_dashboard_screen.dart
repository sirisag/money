...
import 'package:money/screens/driver/import_initial_data_screen.dart';
// Import screens for recording transactions (to be created)
import 'package:money/screens/driver/record_monk_transaction_screen.dart';
import 'package:money/screens/driver/record_trip_expense_screen.dart';
import 'package:money/services/database_helper.dart';
import 'package:money/models/account_balance_models.dart';
import 'package:intl/intl.dart'; // For number formatting
import 'package:money/screens/driver/monk_transaction_history_screen.dart'; // Import history screen
import 'package:money/screens/driver/driver_transaction_history_screen.dart'; // Import driver's own history screen
import 'package:money/models/user_model.dart'; // For User model
import 'package:money/models/transaction_model.dart'; // For Transaction model
import 'package:money/services/file_export_service.dart'; // For FileExportService
import 'package:money/screens/driver/manage_monks_by_driver_screen.dart'; // Changed to ManageMonksByDriverScreen
import 'package:share_plus/share_plus.dart'; // For sharing files
import 'package:money/utils/constants.dart'; // For AppConstants
...
class MonkForExportDialog {
  ...
  MonkForExportDialog(
      {required this.monk, this.lastActivity, required this.balanceWithDriver});
}
...
class _DriverDashboardScreenState extends State<DriverDashboardScreen> {
  ...
  void _navigateToManageMonksByDriver() async {
    final result = await Navigator.push(
      ...
      MaterialPageRoute(
          builder: (context) =>
              const ManageMonksByDriverScreen()), // Changed to ManageMonksByDriverScreen
    );
    if (result == true && mounted) {
  ...
  Future<void> _showExportDataDialog() async {
    ...
    final List<MonkFundAtDriver> monkFunds =
        await _dbHelper.getMonkFundsByDriver(_currentDriverId!);
    ...
    for (var fund in monkFunds) {
      if (fund.balance != 0) {
        ...
        if (monkUser != null) {
          ...
          monksForDialog.add(MonkForExportDialog(
              monk: monkUser,
    ...
    final TextEditingController advanceReturnController =
            ...
            content: SizedBox(
              ...
              child: Form(
                ...
                child: ListView(
                  shrinkWrap: true,
                  children: <Widget>[
                    const Text('เลือกพระที่จะส่งข้อมูลปัจจัย:',
                        ...
                        onChanged: (bool? value) {
                          setDialogState(() {
                            if (value == true) {
                              ...
                              selectedMonksToExport.removeWhere((m) =>
              ...
              TextButton(
                ...
                onPressed: () async {
                  if (formKey.currentState!.validate()) {
                    ...
                    final int advanceToReturn =
                        int.parse(advanceReturnController.text);
                      ...
                      builder: (BuildContext summaryDialogContext) {
                        return AlertDialog(
                          ...
                          content: SingleChildScrollView(
                            child: ListBody(
                              children: <Widget>[
                                ...
                                Text(// Use the calculated total here
                          ...
                          actions: <Widget>[
                            TextButton(
                                onPressed: () =>
                                    Navigator.of(summaryDialogContext)
                                        .pop(false),
                                child: const Text('ยกเลิก')),
                            TextButton(
                                onPressed: () =>
                    ...
                    if (confirmExport == true && mounted) {
                      ...
                      await _proceedWithDriverDataExport(
                          selectedMonksToExport, advanceToReturn);
                    }
                    // }
                  }
                },
  ...
  Future<void> _proceedWithDriverDataExport(
      List<User> monksToExport, int advanceReturned) async {
    if (_currentDriverId == null) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      final treasurerPrimaryId =
          prefs.getString(AppConstants.associatedTreasurerPrimaryId);
      final treasurerSecondaryId =
          prefs.getString(AppConstants.associatedTreasurerSecondaryId);
    ...
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        final String? driverPrimaryId =
            prefs.getString(AppConstants.userPrimaryId);
        ...
        await db.transaction((txn) async {
          ...
          if (newAdvanceData != null && newAdvanceData['amount'] != null) {
            ...
            if (advanceAmount > 0) {
              DriverAdvanceAccount? currentAdvance =
                  await _dbHelper.getDriverAdvance(driverPrimaryId, txn: txn);
              ...
              await _dbHelper.insertOrUpdateDriverAdvance(currentAdvance,
                  txn: txn);
              ...
              final receiveAdvanceTx = Transaction.create(
                ...
                note: newAdvanceData['note'] as String? ??
          ...
          if (updatedMonkList != null) {
            for (var monkData in updatedMonkList) {
              final monk = User.fromMap(monkData as Map<String,
                  dynamic>); // Ensure User.fromMap handles all fields correctly
              await _dbHelper.insertUser(monk,
                  txn:
                      txn); // Assumes insertUser handles updates if user exists
            }
          }
        }); // End transaction
          ...
          children: <Widget>[
                ...
                child: Column(
                  children: [
                    ...
                    _isLoadingBalance
                        ...
                        : Text(
                            _currencyFormat
                                .format(_driverAdvanceAccount?.balance ?? 0),
            ...
            if (_initialDataImported) ...[
              ElevatedButton.icon(
                ...
                onPressed:
                    _isProcessingFile ? null : _navigateToRecordMonkTransaction,
              ...
              ElevatedButton.icon(
                ...
                onPressed:
                    _isProcessingFile ? null : _navigateToRecordTripExpense,
                style: ElevatedButton.styleFrom(
                    minimumSize: const Size(double.infinity, 48),
                    padding: const EdgeInsets.symmetric(vertical: 12)),
              ),
