...
class _MonkTransactionHistoryScreenState
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      for (var monk in activeMonks) {
        ...
        summaries.add(MonkSummaryCardData(
          monk: monk,
  ...
  void _navigateToDetailedHistory(User monk) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => DetailedMonkHistoryScreen(
          monk: monk,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('สรุปข้อมูลพระ (คนขับรถ)'),
      ),
      body: _isLoading
          ? const Center(child: CircularProgressIndicator())
          : _monkSummaries.isEmpty
              ? const Center(
                  child: Text('ไม่มีข้อมูลพระที่มีกิจกรรมกับคุณ',
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: CircleAvatar(
                          child: Text(monk.displayName.isNotEmpty
                              ? monk.displayName[0]
                              : 'P'),
                        ),
