...
class _ManageMonksByDriverScreenState extends State<ManageMonksByDriverScreen> {
  ...
  Future<void> _loadAssociatedTreasurerIds() async {
    ...
    setState(() {
      _associatedTreasurerPrimaryId =
          prefs.getString('associated_treasurer_primary_id');
      _associatedTreasurerSecondaryId =
          prefs.getString('associated_treasurer_secondary_id');
    });
  }
    ...
    try {
      if (isNew || monkToSave == null) {
        ...
        final updatedMonk = monkToSave.copyWith(
            displayName:
                displayName); // Only display name can be edited here by driver
        await _dbHelper.updateUser(updatedMonk);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content:
                  Text('แก้ไขข้อมูลพระ "${updatedMonk.displayName}" สำเร็จ')),
        );
      }
      if (mounted) {
                ...
                children: <Widget>[
                  if (_isAddingNewMonk)
                    TextFormField(
                      ...
                      decoration:
                          const InputDecoration(labelText: 'ชื่อ/ฉายาของพระ'),
                      validator: (value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('จัดการบัญชีพระ (โดยคนขับรถ)'),
      ),
      body: _isLoading
