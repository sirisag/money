...
import 'package:money/models/account_balance_models.dart'; // For MonkFundAtDriver
...
class _DriverTransactionHistoryScreenState
  ...
  Future<void> _loadDriverTransactions() async {
    ...
    try {
      final transactionsFromDb =
          await _dbHelper.getTransactionsByRecordedUser(_currentDriverId!);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('ประวัติธุรกรรมของฉัน'),
      ),
      body: _isLoading
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (transaction.type ==
                      ...
                      amountPrefix = "+ ";
                      final monk =
                          _monkDetailsCache[transaction.sourceAccountId];
                      if (monk != null) {
                        titleText =
                            '${monk.displayName} (ID: ${monk.primaryId})';
                        firstLineDetail =
