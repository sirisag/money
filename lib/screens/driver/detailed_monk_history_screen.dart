...
class _DetailedMonkHistoryScreenState extends State<DetailedMonkHistoryScreen> {
  ...
  Future<void> _loadTransactionsForMonk() async {
    ...
    try {
      final transactionsFromDb = await _dbHelper.getMonkHistoryForDriverView(
          widget.monk.primaryId, widget.driverId);
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('ประวัติธุรกรรม: ${widget.monk.displayName}'),
      ),
      body: _isLoading
          ? const Center(child: CircularProgressIndicator())
          : _transactions.isEmpty
              ? const Center(
                  child: Text('ไม่มีรายการธุรกรรม',
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading:
                            Icon(transactionIcon, color: amountColor, size: 30),
