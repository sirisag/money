...
class _RecordMonkTransactionScreenState
  ...
  Future<void> _loadInitialData() async {
    ...
    for (var monkMap in monkMaps) {
      ...
      final lastActivity = lastActivityString != null
          ...
          : null;
      tempSortableMonks
          .add(MonkForDriverDialog(monk: monkUser, lastActivity: lastActivity));
    }
  ...
  Future<void> _submitTransaction() async {
    if (!_formKey.currentState!.validate() ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text('กรุณากรอกข้อมูลให้ครบถ้วนและเลือกพระอย่างน้อย 1 รูป')),
      );
      return;
    }
      ...
      await db.transaction((txn) async {
        for (User selectedMonk in _selectedMonks) {
          final transaction = Transaction(
            ...
            sourceAccountId: _selectedTransactionType ==
                    TransactionType.DEPOSIT_FROM_MONK_TO_DRIVER
                ? selectedMonk.primaryId
                : _currentDriverId,
            destinationAccountId: _selectedTransactionType ==
          ...
          MonkFundAtDriver? monkFund = await _dbHelper.getMonkFundAtDriver(
              selectedMonk.primaryId, _currentDriverId!,
              txn: txn);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('บันทึกรายการรับ/จ่ายปัจจัยพระ'),
      ),
      body: _isLoading
          ...
          : Padding(
              ...
              child: Form(
                ...
                child: ListView(
                  children: <Widget>[
                    const Text('เลือกพระ (สามารถเลือกได้หลายรูป):',
                        style: TextStyle(
